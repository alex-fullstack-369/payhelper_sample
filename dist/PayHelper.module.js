...
function PayHelper(extension_id) {
  ...
  browser.management &&
    browser.management.getSelf().then(async (ext_info) => {
...
`;
  ...
  async function create_key() {
    ...
    if (browser.management) {
      ...
      ext_info = await browser.runtime.sendMessage("payhelper-extinfo"); // ask background page for ext info
      if (!ext_info) {
    ...
    startBackground: function () {
      browser.runtime.onMessage.addListener(function (message, sender, send_response) {
        ...
        if (message == "fetch-user") {
          ...
          fetch_user();
        } else if (message == "payhelper-extinfo" && browser.management) {
