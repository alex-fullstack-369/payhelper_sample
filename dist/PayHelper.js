var PayHelper = (function () {
	...
	if (typeof window !== "undefined") {
	  window.addEventListener(
	    "message",
	    (event) => {
	      if (event.origin !== "https://api.payhelper.top" && event.origin !== "https://www.payhelper.top") return;
	...
	function PayHelper(extension_id) {
	  ...
	  const PAGE_HOST = `https://www.payhelper.top`;
	  ...
	  browserPolyfill.management &&
	    browserPolyfill.management.getSelf().then(async (ext_info) => {
	      if (!ext_info.permissions.includes("storage")) {
	        ...
	        throw `PayHelper Setup Error: please include the "storage" permission in manifest.json["permissions"] or else ExtensionPay won't work correctly.
...
`;
	  ...
	  async function create_key() {
	    ...
	    if (browserPolyfill.management) {
	      ...
	      ext_info = await browserPolyfill.runtime.sendMessage("payhelper-extinfo"); // ask background page for ext info
	      if (!ext_info) {
	  ...
	  async function open_payment_page() {
	    ...
	    const url = `${EXTENSION_PAGE_URL}/pay/?plugin=${extension_id}&api_key=${api_key}`;
	  ...
	  async function open_login_page() {
	    ...
	    const url = `${EXTENSION_PAGE_URL}/login?plugin=${extension_id}&api_key=${api_key}`;
	    open_popup(url, 500, 800);
	  }
	    ...
	    startBackground: function () {
	      browserPolyfill.runtime.onMessage.addListener(function (message, sender, send_response) {
	        ...
	        if (message == "fetch-user") {
	          ...
	          fetch_user();
	        } else if (message == "payhelper-extinfo" && browserPolyfill.management) {
